...
export const arcjetProtection = async (requestAnimationFrame,resizeBy,next) => {
    try{
        const decision = await aj.protect(req);
        if(decisionisDenied()){
            if(decision.reason.isRateLimit()){
                return resizeBy.status(429).json({message: "Rate limit exceed. Please try again later."});
            }
